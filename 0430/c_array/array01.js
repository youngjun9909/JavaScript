...
let fruits = ['Apple', 'Banana', 'Mango']; // 문자열 배열
let numbers = [1, 2, 3, 4, 5]; // 숫자형 배열
let emptyArray = []; // 빈 배열

// 팁) 콜론(:)이나 콤마(,)의 경우 앞선 문자와는 붙여쓰기, 뒤의 문자와는 띄어쓰기를 권장

//& 2) Array 생성자를 사용한 배열 생성
// : 새로운 배열을 만들 때 배열의 크기나 초기 값을 지정하는 데 유용

let arrayConstructor = new Array(3); // 크기가 3인 배열 생성
let arrayFruits = new Array('사과', '바나나', '망고');
...
// 배열의 길이를 변경
// : 기존의 배열의 길이를 줄이면 줄이는 만큼의 요소가 배열의 끝에서부터 잘림
sports.length = 2; // 길이 축약
console.log(sports); // [ '축구', 'baseball' ]

// : 기존의 배열의 길이를 늘리면 새로운 요소는 undefined로 초기화
sports.length = 4;
console.log(sports); // [ '축구', 'baseball', <2 empty items> ] - undefined의 값이 2개
...
fruits.splice(1, 2, 'Banana', 'Apple'); // 'Mango', 'Peach' 대신 'Banana', 'Apple'로 교체
...
let mixNumbers = [52, 32, 15, 74, 43];
console.log(mixNumbers.sort()); // [ 15, 32, 43, 52, 74 ]

// reverse(): 배열의 요소의 순서를 반대로 정렬(내림차순)
let reverseArray = fruits.reverse();
console.log(reverseArray); // [ 'Strawberry', 'Banana', 'Apple' ]

console.log(mixNumbers.reverse()); // [ 74, 52, 43, 32, 15 